...
Day 23's puzzle is a variation of the classic [Towers of Hanoi](https://en.wikipedia.org/wiki/Tower_of_Hanoi) problem.
Instead of disks, the puzzle has players move different types of [amphipod](https://en.wikipedia.org/wiki/Amphipoda), in keeping with the year's ocean theme. 
The goal is to move each amphipod of each type into the 
...
A post on the [subreddit](https://www.reddit.com/r/adventofcode/comments/wl873a/aoc_and_professional_developers/) suggested using the A* ("A star") algorithm. [A*]()https://en.wikipedia.org/wiki/A*_search_algorithm) is a classic pathfinding algorithm that calculates the shortest path between two 
given nodes of a [graph](https://en.wikipedia.org/wiki/Graph_(discrete_mathematics)). Implementations rely on two helper functions. `d` measures distances 
between nodes, and a heuristic function called `h` estimates the distance between 
a node and the goal. The algorithm is mathematically certain to return the 
correct path so long as `h` never overestimates the distance to the goal. 
...
is acyclic --- loops are impossible). Also, I only needed to find the 
distance of the shortest path, not the path itself.
...
are discouraged from sharing).
I made the crucial decision to represent positions on the board as tuples of `(x, y)` coordinates.
Since I was using Python, I decided to use a zero-based index, with the leftmost hall space as the `x` origin and the bottom side room spaces as the `y` origin. So the leftmost `A` amphipod on the board above would be located at `(2, 2)`. Using complex numbers instead of tuples would have made my life much easier. In any 
...
Then I wrote a complicated that computed the coordinates of the spaces between each pair of coordinates it was legal to move between.
That way, I could discard moves if I detected an amphipod blocking them.
...
4. A side room may only admit amphipods if has an open space and contains no amphipods 
   of a different type.
...
and several times regretted starting. Then, as I worked one fine Monday morning in July, the code spat out a plausible-looking answer. Not expecting success, 
...
a larger game board, crossed my fingers, and ran the script again. It took a few minutes longer, but it returned the correct answer for part 2. I had done it. 
