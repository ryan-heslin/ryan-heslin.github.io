...
When I started planning this blog, I intended to create it using [R Markdown](https://rmarkdown.rstudio.com/) with [Blogdown](https://bookdown.org/yihui/blogdown/). But right 
before I committed, I learned about [Quarto](https://quarto.org/), a 
new Markdown variant with intriguing features. I decided to go with Quarto, 
...
does. First, you write a code chunk in your document like this:
...
you can write all kinds of math formulae, like this set identity: 

$$
|A \cup B| = |A| + |B| - |A \cap B|
$$

I learned R Markdown format before 
my first semester of grad school, and I used it from the start to make reports to my assistantship supervisor.
...
you had to run the render command in R by executing `R -e 'rmarkdown::render("thefile.Rmd")'` or similar. 
Many people used R Markdown to write blogs, but no built-in support existed, 
forcing them to rely on external packages like Blogdown. Most obviously, 
...
a ubiquitous format in data science,
...
offers a cleaner interface for setting chunk options than R Markdown.
Most importantly for my use case, Quarto was designed to support 
blog editing natively. [The docs even have a step-by-step guide](https://quarto.org/docs/websites/website-blog.html).  
...
any needed tweaks, then repeat the process. Once satisfied, I 
